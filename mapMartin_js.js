...
function initMap() {
    ...
    var austin = { lat: 30.2870271, lng: -97.7291743 };
    ...
    //function to calculate miles to meters conversion

    var milesToMeters = function(miles) {
        radius = miles * 1609;
    };

    //function to make ajax call that gathers places in a radius on submit click
    //eventually have radius conneted to d
    var radius = 2.5;
    milesToMeters(radius);
    var ajaxUrl = "https://maps.googleapis.com/maps/api/place/radarsearch/json?";
    ...
    console.log(ajaxUrl + startPoint + ajaxRadius + type + key);
var radiusCall = function(){
    $.ajax({
        url: (ajaxUrl + startPoint + ajaxRadius + type + key),
        method: "GET",
        dataType: 'json'
    }).done(function(data) {
        console.log(data);
    ...
    //function to make ajax call that gathers information about places from radius





    //function to push ajax call information at end of click to infowindow
